  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-950/50 backdrop-blur-sm flex items-center justify-center">
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-8 w-full max-w-sm">
        ...
        <button
          onClick={() => signIn("github-viewer", { callbackUrl: "/" })}
        ...
        <p className="text-xs text-gray-500 mt-2 text-center">
          Read-only access to your profile and email. You&apos;ll be able to
          view repos, jobs, and PRs but not make changes.
        </p>
        ...
        <button
          onClick={() => signIn("github-admin", { callbackUrl: "/" })}
          className="w-full bg-transparent hover:bg-gray-800 border border-gray-700 text-gray-300 text-sm font-medium py-2 rounded-lg transition-colors flex items-center justify-center gap-2"
        >
          <GitHubIcon />
          Sign in as admin
        </button>
        <p className="text-xs text-gray-500 mt-2 text-center">
          Full repo access. Required for adding repos, running jobs, and
          opening PRs.
        </p>
      </div>
    </div>
